...
const AGENTS_ALL = ["implement", "check", "research", "unit-test", "api-test"]
const AGENTS_REQUIRE_TASK = ["implement", "check", "unit-test", "api-test"]
...
/**
 * Get context for test agents (unit-test / api-test). `taskDir` may be
 * relative or absolute.
 *
 * Read order:
 *   1. All files in test.jsonl (use-case design manifest + test specs)
 *   2. test/用例设计.md (use case design doc, explicit materialization so
 *      it is injected even when test.jsonl is missing)
 *   3. prd.md (requirements)
 *   4. design.md if present (technical design)
 *   5. implement.md if present (execution plan)
 * All blocks share one total budget like implement/check contexts.
 */
function getTestContext(ctx, taskDir) {
  const parts = []
  const taskDirFull = ctx.resolveTaskDir(taskDir)
  if (!taskDirFull) return ""
  ...
  // 1. Read test.jsonl
  const jsonlPath = join(taskDirFull, "test.jsonl")
  const blocks = ctx.readJsonlWithFiles(jsonlPath, limits, budget)
  if (blocks.length > 0) {
    parts.push(ctx.buildContextFromEntries(blocks))
  }

  // 2. Use case design document (explicit, works without test.jsonl)
  const useCaseBlock = materializeArtifact(
    ctx.directory,
    `${taskDir}/test/用例设计.md`,
    `${taskDir}/test/用例设计.md (Use Case Design)`,
    "Use case design document",
    limits,
    budget,
  )
  if (useCaseBlock) parts.push(useCaseBlock)
...
**Forbidden**: Suggest improvements, criticize implementation, modify files`,
    ...
    "unit-test": `<!-- trellis-hook-injected -->
# 单元测试 Agent 任务
...
1. **读取上下文** - 以上 test.jsonl 条目、用例设计文档、prd/design/implement 均已注入
2. **编写测试脚本** - 依据 test/用例设计.md 的 UT 用例，沿用项目现有测试框架与风格，写入 {TASK_DIR}/test/unit/
3. **两轮修复循环** - Round 1 运行 → 修复（测试脚本或产品代码）→ 报告 report-round-1.md；未全绿 → Round 2 → report-round-2.md
4. **停止条件** - 全部通过即结束；两轮后仍有失败则停止，报告交回主会话，不继续自修

## 重要约束

- 轮次上限 2，每轮必须生成独立中文报告到 {TASK_DIR}/test/unit/
- 不得分发任何其他 sub-agent
- 不得执行 git commit
- 不跳过测试（禁 skip/only）、不为修绿删断言`,

    "api-test": `<!-- trellis-hook-injected -->
# 接口测试 Agent 任务
...
1. **读取上下文** - 以上 test.jsonl 条目、用例设计文档、prd/design/implement 均已注入
2. **编写测试脚本** - 依据 test/用例设计.md 的 IT 用例（HTTP/RPC、CLI 命令、模块公共 API 以用例设计定义为准），沿用项目现成测试工具，写入 {TASK_DIR}/test/api/
3. **两轮修复循环** - Round 1 运行 → 修复（测试脚本或产品代码）→ 报告 report-round-1.md；未全绿 → Round 2 → report-round-2.md
4. **停止条件** - 全部通过即结束；两轮后仍有失败则停止，报告交回主会话，不继续自修

## 重要约束

- 轮次上限 2，每轮必须生成独立中文报告到 {TASK_DIR}/test/api/
- 测试环境妥善管理：起停的服务自行清理，不留遗留进程/资源
- 不得分发任何其他 sub-agent
- 不得执行 git commit
- 不跳过测试（禁 skip/only）、不为修绿删断言`
  }
      ...
      "tool.execute.before": async (input, output) => {
        try {
          ...
          switch (subagentType) {
            ...
            case "unit-test":
            case "api-test":
              context = getTestContext(ctx, taskDir)
              break
          }
