...
In OpenCode, every unresolved user decision and the final review confirmation must call the OpenCode `question` tool; never ask only in prose. Submit one question at a time with 2–3 mutually exclusive options. Put the recommended option first, explain each option's trade-off, and keep OpenCode's free-form Other answer available.
...
```bash
TASK_DIR=$(/usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin//usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin/python3 ./.trellis/scripts/task.py create "<short task title>" --slug <slug>)
```
