...
### OpenCode current-activity snapshot

OpenCode's `.opencode/plugins/inject-workflow-state.js` also observes structured
Skill, Task/sub-agent, and explicit file-edit tool calls when
`TRELLIS_WORKFLOW_STATE=1` and a safe `TRELLIS_CONTEXT_ID` are present. It writes
one atomic snapshot per context to
`.trellis/.runtime/workflow-state/<context-id>.json` through
`.opencode/lib/workflow-state-store.js`.

The snapshot is an activity hint, not a workflow result: it never records
prompts, replies, commands, or tool output; it does not mark phases completed;
and sub-agent exit only clears the active role. Consumers must use their own
process status to decide whether execution is active, idle, or failed.
