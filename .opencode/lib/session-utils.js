...
const PYTHON_CMD = platform() === "win32" ? "python" : "/usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin/python3"
...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (active.stale || !taskDir || !existsSync(taskDir)) {
    return `Status: STALE POINTER\nTask: ${taskRef}\nNext-Action: Task directory not found. Run: /usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin/python3 ./.trellis/scripts/task.py finish`
  }
...
function buildCompactCurrentState(ctx, platformInput, specIndexPaths) {
  ...
  if (existsSync(tasksDir)) {
    try {
      ...
      lines.push(`Active tasks: ${activeTasks.length} total. Use \`/usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin/python3 ./.trellis/scripts/task.py list --mine\` only if needed.`)
    } catch {
...
Trellis compact SessionStart context. Use it to orient the session; load details on demand.
  ...
  if (workflowContent) {
    ...
    const overviewLines = [
      ...
      "Full guide: .trellis/workflow.md. Step detail: `/usr/local/lib/node_modules/@mindfoldhq/trellis/portable/python/bin/python3 ./.trellis/scripts/get_context.py --mode phase --step <X.Y>`.",
      "",
    ]
